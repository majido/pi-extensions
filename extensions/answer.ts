/**
 * Originally from: https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/answer.ts
 * Copyright Armin Ronacher, licensed under Apache License 2.0
 *
 ...
 * 1. /answer [optional extraction guidance] gets the last assistant message
...
const BASE_SYSTEM_PROMPT = `You are a question extractor. Given text from a conversation, extract any questions that need answering.
...
function buildSystemPrompt(extractionGuidance?: string): string {
	const guidance = extractionGuidance?.trim();
	if (!guidance) {
...
Use this guidance only to decide what should become answerable items. It may ask you to treat statements, checklist items, decisions, options, or recommendations as questions. When the guidance points at non-question text, convert each relevant item into a concise question for the user to answer. Do not answer the questions yourself.`;
}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext, extractionGuidance?: string) => {
			if (!ctx.hasUI) {
			...
			const systemPrompt = buildSystemPrompt(extractionGuidance);
	...
	pi.registerCommand("answer", {
		description: "Extract questions from last assistant message into interactive Q&A. Optional args guide extraction.",
